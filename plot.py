        ...
        for line in file:
            ...
            for auction_id, auction in data['prev_auctions'].items():
                ...
                for bid in auction.get('bids', []):
                    bids.append({
                        'round': round_num - 1,  # Bids occurred in the previous round
                        'auction_id': auction_id,
                        'bot_id': bid['a_id'],
                        'gold_bid': bid['gold'],
                        'reward': reward,
                        'expected_value': auction['num'] * (auction['die'] + 1) / 2 + auction['bonus']
                    })
...
# Load the data
states_df, auctions_df, bids_df = parse_logs('auction_house_log_round_1.jsonln')

# Dash Layout
app.layout = html.Div(style={'fontFamily': 'Arial, sans-serif', 'padding': '20px'}, children=[
    html.H1("Auction Bot Performance Analysis", style={'textAlign': 'center', 'color': '#4A90E2'}),
    
    dcc.Dropdown(
        id='bot-selector',
        options=[{'label': bot_name_mapping[bot_id], 'value': bot_id} for bot_id in bot_name_mapping],
        value=['e302b33b9d7a134e980b854068970d59059397dde8fcbde722bdc1921adaca16'],
        multi=True,
        style={'width': '60%', 'margin': 'auto'}
    ),
    
    html.Div(id='graphs-container', style={'display': 'flex', 'flexDirection': 'column', 'gap': '20px'}),
...
def update_bid_success_rate(selected_bots):
    ...
    for bot_id in selected_bots:
        bot_name = bot_name_mapping.get(bot_id, bot_id)  # Use name from mapping
        bot_bids = bids_df[bids_df['bot_id'] == bot_id]
        success_rate = (bot_bids['reward'] > 0).mean()  # Calculate success rate
        fig.add_trace(go.Bar(
            x=[bot_name],
            y=[success_rate],
            name=bot_name
